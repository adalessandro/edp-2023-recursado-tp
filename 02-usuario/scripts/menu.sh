...
do
 ...
 [ $REPLY == 1 ] && read -p "Escriba la ruta de los archivos: " ruta_archivos && ./clasificar.sh "$ruta_archivos" && continue        # Llama al script clasificar.sh con la ruta proporcionada
 [ $REPLY == 2 ] && monitor_sistema && continue
 [ $REPLY == 3 ] &&  analizar_texto && continue
